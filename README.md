...
```bash
# Linux (x86_64)
curl -sSL https://github.com/angelnicolasc/graymatter/releases/download/v0.14.0/graymatter_0.14.0_linux_amd64.tar.gz | tar -xz && sudo mv graymatter /usr/local/bin/

# macOS (Apple Silicon)
curl -sSL https://github.com/angelnicolasc/graymatter/releases/download/v0.14.0/graymatter_0.14.0_darwin_arm64.tar.gz | tar -xz && sudo mv graymatter /usr/local/bin/

# Windows (PowerShell)
iwr https://github.com/angelnicolasc/graymatter/releases/download/v0.14.0/graymatter_0.14.0_windows_amd64.zip -OutFile graymatter.zip
...
| Voyage AI | `VOYAGE_API_KEY` set — Anthropic has no embeddings API; this slot used to dial a nonexistent Anthropic endpoint and silently degrade to keyword-only (fixed in v0.15) |
...
*GrayMatter — v0.14.0 — August 2026*
