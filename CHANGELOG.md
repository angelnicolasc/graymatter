...
> Identical contents to 0.16.1/0.16.2, which never produced a release: goreleaser v2 dropped the `gomod.proxy` build option this pipeline initially relied on, and the freshly pushed tag was not yet indexed by the module checksum database for the plain build either. The workflow now applies a temporary `replace` to the build checkout and reverts it after the release — the published `go.mod` stays replace-free and `go install ...@v0.16.3` resolves the tagged versions. 0.16.3 is the first version published with binaries.
