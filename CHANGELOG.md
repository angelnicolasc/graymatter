...
- **`benchmarks/hook_latency` — the hook budgets are machine-checked, not aspirational** (every published number must be). The gate builds the real binary, seeds a 10k-fact store, fires the hook as a fresh OS process per sample through the daemon (the shape Claude Code uses), and gates the hook-internal time: user-prompt < 150 ms p99, pre-compact < 200 ms, session-end < 500 ms — with end-to-end wall time reported transparently. Verified injections are counted so the benchmark cannot silently measure the throttle instead of recall. `graymatter bench --hooks` audits the same published budgets against this binary itself — the CLI is the hook binary, nothing compiles — so the numbers stay machine-checked wherever the product runs, not only in CI. Getting under the gate took three behaviour-preserving optimizations to the recall path: `keywordScore` now tokenizes each fact once instead of twice (byte-identical scores), the pipeline decodes through a lite fact decoder that skips the fields ranking never reads (`unmarshalFactLite`, pinned against the full decoder by a 500-case agreement test), and recency ranks are read straight from the newest-first list order instead of being re-sorted — recency is strictly monotonic in `CreatedAt`, so the rank order was already there, and equal-`CreatedAt` facts (Windows clocks tick at ~15.6 ms) now resolve by ID deterministically instead of by `sort.Slice`'s unspecified tie order.
