...
Extensi�n medida por `go run ./benchmarks/retrieval_quality` sobre las queries
congeladas `queries-multihop-v1.jsonl` (q7�q9): preguntas cuyo gold fact no
comparte NING�N t�rmino superficial con la query � solo alcanzables mediante un
...
Cobertura de puente: **1/78 facts** producen entidades extra�bles por el
...
**Veredicto: NO-GO para wiring este ciclo.** La condici�n bloqueante no es el
algoritmo de expansi�n sino la cobertura del extractor sobre prosa t�cnica real
...
determinador, Unicode, puntuaci�n final de URLs), luego re-corrida de ambos
benchmarks. Mientras tanto el grafo sigue siendo �til v�a linking expl�cito
...
corpus-v1 no ten�a entidades recurrentes entre facts (cada filler era una
oraci�n �nica), as� que la condici�n 2 era estructuralmente imposible ah�.
...
Run exploratorio (pre-registro formal se har� sobre los n�meros finales de
wiring):
...
?p95 � -0.5ms (ruido). Las tres queries multi-hop se responden v�a puente de
entidad: la query trae el fact-hub, el hub comparte entidad con el gold, la
expansi�n lo incorpora. Condiciones 2�4 del gate ADR-003: PASS.
